...
def sample_data_only_4(ecapa_emb, s2pecnet_emb, lcnn_stft_emb, lcnn_cqt_emb, aasist_emb, dataset) :
    X_data = []
    y_label = []

    for data in dataset :
        target, second, label = data
        emb_concat = np.concatenate((ecapa_emb[target], ecapa_emb[second],aasist_emb[second], s2pecnet_emb[second]), axis= 0)
...
def sample_data_only_2(ecapa_emb, s2pecnet_emb, lcnn_stft_emb, lcnn_cqt_emb, aasist_emb, dataset) :
    X_data = []
    y_label = []

    for data in dataset :
        target, second, label = data
        emb_concat = np.concatenate((ecapa_emb[target], ecapa_emb[second]), axis= 0)
